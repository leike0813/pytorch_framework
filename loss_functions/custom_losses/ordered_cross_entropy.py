...
class OrderedCrossEntropyLoss(nn.Module):
    def __init__(self, num_classes, class_values=[], weight=None, reduction='mean', label_smoothing=0.0):
        ...
        if len(class_values) == 0:
            self.register_buffer('class_values', torch.tensor([i for i in range(num_classes)], dtype=torch.float32))
            ...
            self.register_buffer('class_values', torch.tensor(class_values, dtype=torch.float32).to('cuda' if torch.cuda.is_available() else 'cpu'))
    ...
    def forward(self, input, target):
        if self.class_values.device != input.device:
            self._to_device(input.device)
